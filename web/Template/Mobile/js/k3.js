...
$(function(){
	...
	$(".my_close_lishi").eq(0).unbind().click(function () {
		$("#fn_getoPenGame").toggle(); 
	});
	$(".my_close_lishi2").eq(0).unbind().click(function () {
		$("#fn_getoPenGame").toggle(); 
	});
		...
		if(data.sign==true){
			...
			for(var o in lotlist){
				...
				if(sum%2!=0)
					...
					codesarrs = openinfo.opencode.split(',').sort();
					codespan = "";
					for(var a = 0; a<codesarrs.length;a++){
						if(codesarrs[a] == "1"){
							codespan+="<span id='myopencode' class='open_numb_1'></span>";
						}else if(codesarrs[a] == "2"){
							codespan+="<span id='myopencode' class='open_numb_2'></span>";
						}else if(codesarrs[a] == "3"){
							codespan+="<span id='myopencode' class='open_numb_3'></span>";
						}else if(codesarrs[a] == "4"){
							codespan+="<span id='myopencode' class='open_numb_4'></span>";
						}else if(codesarrs[a] == "5"){
							codespan+="<span id='myopencode' class='open_numb_5'></span>";
						}else if(codesarrs[a] == "6"){
							codespan+="<span id='myopencode' class='open_numb_6'></span>";
						}
					}
				html += '<tr style="color:#caebda;"><td class="fz13">'+openinfo.expect+'</td><td class="fb">'+codespan+'</td><td class="fb">'+sum+'</td><td>'+smallbig+'&nbsp;|&nbsp;'+oddeven+'</td></tr>';
            ...
            if (msg.sign == true) {
                if (msg.data.opencode.length > 0) {
                    ...
                    if (lastExpect == openexpect) {
						...
						var openCodes = openCode.split(',').sort();
						stopLottery(openCodes.join());
