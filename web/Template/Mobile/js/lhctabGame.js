$(function () {
  ...
  var danshiNumberL = 0;//单式号码长度 selectNumbers
  var yesArr = [];//单式正确的数组
  var orderList= [];//投注数组
  ...
  function updataMoneylhc(){
    ...
    console.log("771"+winningMoney);
    if(!winningMoney){
      winningMoney = $('.lhclm').find('.lhclmPeil').eq(0).find('em').text();
      console.log("774"+winningMoney);
      if(!winningMoney){
        winningMoney = $('.lhcaddMoneyInfo').eq($('.lhcaddMoneyInfo').length - 2).text();
        console.log("777"+winningMoney);
      }
      if(!winningMoney){
        winningMoney = $('.lhclm').find('.lmbbNumber').eq(0).find('em').text();
        console.log("781"+winningMoney);
      }
    }
  ...
  $('.bet_filter_box').on('click','.bet_options',function (){
    ...
    switch(_thisType){
      ...
      case 'lm3z2':
        $('.play_select_prompt').find('span[way-data="tabDoc"]')
          .html('至少选择三个号码，每三个号码为一组合，若其中至少有两个是开奖号码中的正码，即为中奖。若中两码，叫三中二之中二;若三码全中，叫三中二之中三。赔率 <em style="color:red;">'+rates.rate+'</em>元');
        var caizhong = ['中二','中三'];
...
// lhc_red_bg
  ...
  function lhcgameNumber(arr){
    var mybeishu = $('.play_select_prompt').find('span[way-data="tabDoc"] em').text()
    console.log(mybeishu);
    var box = $('.g_Number_Section');
    var lhcstr = '';
    ...
    for(var i = 0;i<arr.length;i++){
      ...
      for(var j = 1;j<50;j++){
        if(j < 10){
          ...
          boxNumber.append('<a style="line-height:22px;" href="javascript:void(0);" class="selectNumber lhc_'+lhcNumberData.bo+'_color" data-bg="lhc_'+lhcNumberData.bo+'_bg" data-number="'+lhcstr+'">'+lhcstr+'<span style="color: #999;font-weight: 400; display: block;font-size: 12px;height: 12px;line-height: 12px;">'+mybeishu+'</span></a>');
        }else{
          lhcstr = j;
          lhcNumberData = eval('lhc_numData.lhc_'+lhcstr);
          boxNumber.append('<a style="line-height:22px;" href="javascript:void(0);" class="selectNumber lhc_'+lhcNumberData.bo+'_color" data-bg="lhc_'+lhcNumberData.bo+'_bg" data-number="'+lhcstr+'">'+lhcstr+'<span style="color: #999;font-weight: 400; display: block;font-size: 12px;height: 12px;line-height: 12px;">'+mybeishu+'</span></a>');
