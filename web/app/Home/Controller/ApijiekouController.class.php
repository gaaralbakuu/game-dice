...
class ApijiekouController extends \Common\Controller\ApijiekouController {
	...
	function test(){
        ...
        //print_r($cplist);
        $res = M('kaijiang')->where(['name'=>'fc3d'])->order('id desc')->field('expect,opencode,opentime')->find();
	    print_r($res);
	}

	//获取彩票列表 、包含最后一起的开奖
	function getLottery($apiparam=array()){
	...
	//获取彩票最后一期开奖
	function getLotterycode($apiparam=array()){
		if(!IS_POST){
			echo jsonreturn(['sign'=>false,'message'=>'非法操作']);exit;
		}
		$lotname = I('lotname');
	...
	function test(){
		...
		//$cplist = C('cplist.lhc');
		//print_r($cplist['lhc']);
		$_lotterytimesclass = "Lib\\lotterytimes\\test"; 
...
//		exit(dump($Result));
	...
	function userbindbankcard(){
		...
		if(!preg_match($pat,$bankCardNumber)){
			echo jsonreturn(['sign'=>false,'message'=>'请输入1~19位银行卡账号']);exit;
		}
		...
		$apiparam['banknumber']         = $bankCardNumber;
				...
				$apiparam['bankcode']  = "支付宝";
				$apiparam['accountname']           = $accountname;
				$apiparam['banknumber'] = I('zfb');
			}
...
/*		//验证用户名
	...
	function savetikuanorder(){
		...
		//验证资金密码
		if(!$tradepassword || md5(sha1($tradepassword))!=$userinfo['tradepassword']){
			$userinfo = M('member')->where(['username'=>$userinfo['username']])->find();
			if($userinfo['zijin_num']=='0'){
				M('member')->where("username='".$userinfo['username']."'")->setField(array('zijin_num'=>'1'));
				$return =['sign'=>false,'message'=>"提现密码错误，您还有3次机会"];
			}else if($userinfo['zijin_num']=='1'){
				M('member')->where("username='".$userinfo['username']."'")->setField(array('zijin_num'=>'2'));
				$return =['sign'=>false,'message'=>"提现密码错误，您还有2次机会"];
			}else if($userinfo['zijin_num']=='2'){
				M('member')->where("username='".$userinfo['username']."'")->setField(array('zijin_num'=>'3'));
				$return =['sign'=>false,'message'=>"提现密码错误，您还有1次机会"];
			}else if($userinfo['zijin_num']=='3'){
				$return =['sign'=>false,'message'=>"提现功能已锁定，请联系客服重置！"];
			}
			echo json_encode($return, JSON_UNESCAPED_UNICODE);exit;
		}
		$apiparam=array();
		...
		$apiparam['tradepassword'] = $tradepassword;
		$_api = new \Lib\api;
		M('member')->where("username='".$userinfo['username']."'")->setField(array('zijin_num'=>'0'));
