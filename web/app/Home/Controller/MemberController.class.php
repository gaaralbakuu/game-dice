...
//修改银行卡
	...
	/*	$this->EndTime   =*/ $EndTime   = I('EndTime');
		...
		//$this->assign('allokamount',$allokamsigninount==0?0:$allokamount);
	...
	function betRecordapi($post_data = null)
	{
	/*	$this->name      =*/ $name      = $post_data['name'];
	/*	$this->atime     =*/ $atime     = $post_data['atime'];
	/*	$this->a_item    =*/ $a_item    = $post_data['a_item'] == ''?1:$post_data['a_item'];
	/*	$this->qihao     =*/ $expect     = $post_data['expert'];
	/*	$this->StartTime =*/ $StartTime = $post_data['StartTime'];
	/*	$this->EndTime   =*/ $EndTime   = $post_data['EndTime'];
		...
		$Page       = new \Think\Page($count,10);
		//startPage($Page);
		$tzlist     = $touzhu->where($map)->order("oddtime desc")->limit($Page->firstRow.','.$Page->listRows)->select();
		return $tzlist;
		//print_r($touzhu);
		//print_r($tzlist);
		//exit;
