...
if((floatval($value['amountafter'])-floatval($value['amountbefor']))>0){
...
if((floatval($value['amountafter'])-floatval($value['amountbefor']))>0){
		...
		foreach ($mxlist as $key => $value) {
			if($value['type']!='xima')
			{
				array_push($mxlist2,$value);
			}
		}
		file_put_contents("test.txt", json_encode($mxlist2));
...
/*		foreach($_paylist as $k=>$v)
    ...
    $yingli = ($zhongjiangcount - $touzhucount +  $fandian +  $fanshui + $ctouzhucount+$jinji+$caijin+$activity_rks);
