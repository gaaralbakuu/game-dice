...
class ActivityController extends CommonController {
	...
	public function show(){
		...
		$this->assign('huodong',$otherhd[$id]['content']);
		$this->assign('title',$otherhd[$id]['title']);
		$this->display();
	}
//公共
...
//					if(($user['groupid']-$jiangli[0]['groupid'])>=2)$amount++;
				...
				$fdata = [];
				$fdata['trano'] = $data['trano'];
				$fdata['uid'] = $this->userinfo['id'];
				$fdata['username'] = $this->userinfo['username'];
				$fdata['type'] = 'jinji';
				$fdata['typename'] = '晋级奖励';
				$fdata['amount'] = $data['jlje'];
				$fdata['amountbefor'] = $user['balance'];
				$fdata['amountafter'] = $user['balance'] + $data['jlje'];
				$fdata['oddtime'] = time();
				$fdata['remark'] = "晋级奖励";
				file_put_contents("money.txt", json_encode($fdata));
			...
			$memint = M('member')->where(['id'=>$this->userinfo['id']])->setInc('balance',number_format($amount,2,".",""));//账户增加金额
			if($int&&$memint){
