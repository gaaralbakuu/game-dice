	...
	function getconfigs($apiparam=array()){
		...
		foreach($_membergrouplists as $k=>$v){
			...
			foreach($getplayers as $k0=>$v0){
			    if(is_array($_xesetconfigs)){
                    $v['min_'.$v0['playid']] = array_key_exists('min_'.$v0['playid'],$_xesetconfigs)?$_xesetconfigs['min_'.$v0['playid']]:0;
                    $v['max_'.$v0['playid']] = array_key_exists('max_'.$v0['playid'],$_xesetconfigs)?$_xesetconfigs['max_'.$v0['playid']]:0;
                }
			}
		...
		foreach($_REQUEST['orderList'] as $k=>$v){
			...
			if(intval($rates[$v['playid']]['maxxf'])>0){
				...
				if(!strstr($_REQUEST['lotteryname'],'k3')){ 
                   if(!strstr($_REQUEST['lotteryname'],'lhc') && !strstr($v['playid'],'lmp_') && !strstr($v['playid'],'dan_') && !strstr($v['playid'],'yzwm') && !strstr($v['playid'],'gyh_') ){
						  //if(($v['price']/$v['zhushu']/$v['beishu']/$v['yjf'])!=2){    //断码每注金额是否正确
                          if(($v['price']*1000/$v['zhushu']/$v['beishu']/($v['yjf']*1000))!=2){
							  $this->ajaxReturn(['sign'=>false,'message'=>'非法操作2'.$v['playid']]);exit;
						  }
