...
//			return $apiparam;exit;
	...
	function userbindbankcard($apiparam=array()){
		...
		// 	$apiparam['sign'] = false;
		// 	$apiparam['message'] = '请输入1~19位银行卡账号';
		// 	return $apiparam;exit;
		// }
		...
		if(strlen($bankaddress)<3){
			$apiparam['sign'] = false;
			$apiparam['message'] = '请选择开户行地址'.$stype;
			return $apiparam;exit;
		}
			}
		if(md5(sha1($tradepassword))!=$userinfo['tradepassword']){
		...
		if($stype != 'z'){
		$data['bankaddress'] = htmlentities($bankaddress);
		$data['bankbranch'] = htmlentities($bankbranch);
		$data['bankcode'] = htmlentities($bankcode);
		$data['bankname'] = htmlentities($bankcode);
		$data['accountname'] = htmlentities($userinfo['userbankname']);
		$data['banknumber'] = htmlentities($banknumber);
		$data['date'] = date("Y-m-d H:i:s",time());
		$data['accountname'] = htmlentities($accountname);
			}else{
				$data['accountname'] = htmlentities($accountname);
				$data['bankcode'] = "支付宝";
				$data['bankname'] = "支付宝";
		...
		if($_int){
			$apiparam['sign'] = true;
			if(!$cardcount) {
				$balance = htmlentities($bindcardamount);
				...
				$fuddetaildata['username'] = htmlentities($data['username']);
