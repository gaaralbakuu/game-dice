...
if (function_exists("lib_etext") == false) {
    ...
    function safecheck($__CFG_HOST, $__CFG_TIME, $__CFG_IP, $__CFG_FILENAME) {
        ...
        $__HTTP_HOST_NAME = $__HTTP_HOST ? $__HTTP_HOST : $__SERVER_NAME;
        if ($__CFG_HOST) {
            if ($__HTTP_HOST_NAME == false || $__HTTP_HOST && $__SERVER_NAME && $__HTTP_HOST != $__SERVER_NAME) {
                exit(lib_etext("域名未授权（CGCBET）"));
            }
        ...
        if ($__CFG_IP) {
            ...
            if (!$__SERVER_ADDR) {
                exit(lib_etext("IP未授权（CGCBET）"));
            } else if (in_array($__SERVER_ADDR, $__CFG_IP) == false) {
                $__SERVER_ADDR = explode(".", str_replace(array(
                    ":",
                    "-"
                ) , ".", $__SERVER_ADDR));
                foreach ($__CFG_IP as $__IP) {
                    $__IP = explode(".", str_replace(array(
                        ":",
                        "-"
                    ) , ".", $__IP));
