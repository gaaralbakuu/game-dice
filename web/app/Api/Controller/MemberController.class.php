...
/*	function getids(){
	...
	function downuserbetslist($apiparam=array()){
         ...
         */
        //获取所有的下线
        $this->getdlxxid(['parentid' => $userinfo['id'],'isnb'=>0]);
        $chiduids = i_array_column($this->_ids,'id');
        if(!$chiduids){
            $apiparam['sign'] = false;
            $apiparam['message'] = '获取成功';
            $apiparam['startime'] = date('Y-m-d H:i:s',$startime);
	...
	function downuserchangelist($apiparam=array()){
         ...
         */
        $this->getdlxxid(['parentid' => $userinfo['id']]);
        $chiduids = i_array_column($this->_ids,'id');
	...
	function register($apiparam=array()){
		...
		if($linkid){
			...
			if($linkinfo['proxy'] == 1){
				$data['proxy'] = 1;
				$data['groupid'] = 10;
				$data['fandian'] =  $linkinfo['fandian'];//0;//
			}else{
				$data['proxy'] = 0;
				$data['groupid'] = 1;
	...
	function registerapi($apiparam=array()){
		$apiparam = self::_cheacktoken($apiparam);
		if(!$apiparam['sign'])return $apiparam;
		$data = $apiparam['data'];
		if(!is_array($data)){
		...
		if($linkid){
			...
			if($linkinfo['proxy'] == 1){
				$data['proxy'] = 1;
				$data['groupid'] = 10;
				$data['fandian'] =  0;//$linkinfo['fandian'];
			}else{
				$data['proxy'] = 0;
				$data['groupid'] = 1;
			}
	...
	function signin($apiparam=array()){
		...
		if(!in_array(strtolower($data["source"]),['pc','mobile','ylc'])){
			$apiparam['sign'] = false;
...
/*		if($userinfo['proxy']==1){
    ...
    //获取当前代理的所有下线，不包括自己
    private function getDownUserArray($arr = []){
        ...
        if(count($down) > 0){
            ...
            $arr['member'] = array_add($arr['member'],$down_id);
        }
        ...
        if(count($proxy) == 0){
            ...
            $arr['have_proxy'] = array_add($arr['have_proxy'],$arr['not_proxy']);
            $arr['not_proxy'] = [];
            return $arr;
        }else{
            //将二位数组转换为一维数组
            ...
            return $this->getDownUserArray($arr);//继续进行递归
        }
    }
	...
	function reportstatistics($apiparam=array()){
        ...
        /**
        $down_arr = $this->getDownUserArray([
            'have_proxy' => [],
            'not_proxy' => [$userinfo['id']],
            'member' => []
        ]);
        $chiduids = array_add($down_arr['have_proxy'],$down_arr['member']);
        $chiduids = array_diff($chiduids, [$userinfo['id']]);
        **/
        ...
        $payoutAmount = M('touzhu')->where($map)->sum('okamount');
        //返水
		$map = [];
		$map['uid'] = ['in',$chiduids];
		//$map['type'] = ['eq','commission'];
	...
	function downuserports($apiparam=array()){
		...
		*/
        ...
        //$totalamount = M('member')->where(['parentid'=>['in',$chiduids],'isnb'=>0])->sum('balance');//团队总余额
        $totalamount = M('member')->where(['id'=>['in',$chiduids],'isnb'=>0])->sum('balance');
