...
class ssc
{
    ...
    */
    ...
    function checkzhushu($playid, $tzcode)
    {
        ...
        $num1 = ['bdw5x1m', 'qwyffs', 'qwhscs', 'qwsxbx', 'qwsjfc', 'bdw4x1m', 'bdwqs','bdwzs','bdwhs'];
        $num2 = ['bdw5x2m', 'hsizxl', 'bdw4x2m', 'bdwqs2m', 'exzuxfsq', 'exzuxfsh','bdwzs2m','bdwhs2m'];
        ...
        $lh = ['lhwq','lhws','lhwg','lhqb','lhqs','lhqg','lhbs','lhbg','lhsg'];
        //信用盘玩法
        $xyp = ['lmp_d1q_da','lmp_d1q_xiao','lmp_d1q_dan','lmp_d1q_shuang',
        ...
        //龙虎
        if(in_array($playid,$lh)){
            return $this->lh($tzcode,1);
        }
        return $this->$playid($tzcode);
    }

    //龙虎
    function lh($tzcode,$num){
        $tzcodes = explode(',', $tzcode);
        foreach ($tzcodes as $k => $v) {
            if ($v > 2 or $v < 0 or !is_numeric($v) or strpos($v, ".")) unset($tzcodes[$k]);
        }
