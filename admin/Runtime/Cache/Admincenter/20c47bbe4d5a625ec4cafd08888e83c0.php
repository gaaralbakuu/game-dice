...
<?php if(is_array($grouplist)): $i = 0; $__LIST__ = $grouplist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$gvo): $mod = ($i % 2 );++$i;?><option value="0" <?php if($gvo['groupid'] == $info['groupid']): ?>selected<?php endif; ?>><?php echo ($gvo["groupname"]); ?>(<?php echo ($gvo["touhan"]); ?>)</option><?php endforeach; endif; else: echo "" ;endif; ?>
</select></span>
			</div>
		</div>
		<div class="row cl">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="" placeholder="代理默认返点9.5%,员默认返点9.0%,最大返点<?php echo GetVar('fanDianMax');?>%" name="fandian">
			</div>
		</div>
		<script>
			$(function(){
