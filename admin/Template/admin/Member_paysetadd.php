...
<article class="page-container">
	<form enctype="multipart/form-data" method="post" name="fileinfo" id="fileinfo" style="width: 200px;font-size: 12px;margin-left: 70px; display: inline-block;">
  	<input type="file" name="file" style="font-size: 14px;" />
	</form>
	<button onclick='send()' style="width: 100px;
    height: 30px;
    margin-top: 5px;
    border: 0;
    font-size: 14px;
    ...
    outline: none;
    color: white;
    background-color: #67C23A;">开始上传</button>
	<script type="text/javascript">
		function send(){
		
	var fd = new FormData(document.getElementById("fileinfo"));
//userName不在表单中
//fd.append("userName", "xiaoxiao");
setTimeout(function(){
...
success:function(data,state){
	alert('上传成功，点击确定后生效！');
document.getElementById("my_code").value = JSON.parse(data).pic;
},
...
<script id="payisonline0" type="text/html">
	...
	<div class="row cl">
		...
		<div class="formControls col-xs-8 col-sm-9">
			<input id="my_code" type="text" class="input-text" value="{$configs.ewmurl}" placeholder="二维码" name="configs[ewmurl]" id="ewmurl">
		</div>
	</div>
</script>
<script id="payisonline1" type="text/html">

    <eq name="info.paytype" value="tg_pay">
        <div class="row cl">
            <label class="form-label col-xs-4 col-sm-3">开启支付类型</label>
            <div class="formControls col-xs-8 col-sm-9 skin-minimal">
                <div class="radio-box">
                    <input name="configs[tgpay_type]" type="radio" value="0" {if condition="$configs.tgpay_type eq 0"}checked{/if}>全部
                </div>
                <div class="radio-box">
                    <input name="configs[tgpay_type]" type="radio" value="1" {if condition="$configs.tgpay_type eq 1"}checked{/if}>微信
                </div>
                <div class="radio-box">
                    <input name="configs[tgpay_type]" type="radio" value="2" {if condition="$configs.tgpay_type eq 2"}checked{/if}>支付宝
                </div>
            </div>
        </div>
    </eq>
