...
class CaipiaoController extends CommonController {
	...
	function yukaijiang(){
		...
		$this->cplist = M('Caipiao')->where()->order('typeid desc')->select();
		foreach($this->cplist as $k=>$v){
			$_cplist[$v['name']] = $v;
		}
		foreach($this->cplist as $k=>$v){
			if($v['name'] != 'lhc'&&$v['name'] != 'fc3d'&&$v['name'] != 'pl3'&&$v['name'] != 'jxk3'){
			$my_list[$v['name']] = $v;
			}
		}
		$this->cplist = $my_list;
		...
		if($actNo>$totalcount){
			...
			if($actNo+19<=$totalcount){
				for($j=$actNo+19,$jj=0;$j>=$actNo;$j--,$jj++){
					$rand_keys = $this->returnrankey($cpinfo['typeid']);
					if($cpinfo['typeid']=='k3' or $cpinfo['typeid']=='keno')sort($rand_keys);
		...
		//---------------官方彩
		$stop_time = $cpinfo['endtime'];
		$first_time = $cpinfo['firsttime'];
		if($cpinfo['name'] == 'dfpk10'){
			$mytime = 300;

		}else if($cpinfo['name'] == 'lhc'){

		}else if($cpinfo['name'] == 'dflhc'){
		...
		$db = M('kaijiang');
		$cur_info = $db->where(['name'=>$cpinfo['name']])->order('expect desc')->find();


		for($a = 1;$a<21;$a++){
			file_put_contents("mytime.txt", date('H:i:s',strtotime($stop_time)) . "==".date('H:i:s',$cur_info['opentime']+$mytime*$a));
			
			$rand_keys = $this->returnrankey($cpinfo['typeid']);
					if($cpinfo['typeid']=='k3' or $cpinfo['typeid']=='keno')sort($rand_keys);
					if(strtotime($first_time)<strtotime((date('H:i:s')))){
					$opentime  = date('Y-m-d H:i:s',$cur_info['opentime']+$mytime*$a);
					}else{
					$opentime  = date('Y-m-d H:i:s',strtotime($first_time)+($mytime*($a-1)));
					}
					if($cpinfo['name'] != "dfpk10"){
					$expect    = $cur_info['expect']+$a;
					}else{
						if(substr($cur_info['expect'],-3) == "180"){
							if($a<10){
							$expect  = date('Ymd')."00".$a;
							}else{
							$expect  = date('Ymd')."0".$a;
							}
						}else{
					...
					$openlist[$expect]['expect'] = $expect;
					$openlist[$expect]['opencode'] = implode(',',$rand_keys);
					$openlist[$expect]['opentime'] = $opentime;
					$openlist[$expect]['cptitle'] = $cur_info['title'];
					$openlist[$expect]['name'] = $cur_info['name'];
					if(date('H:i:s',strtotime($stop_time)) == date('H:i:s',$cur_info['opentime']+$mytime*$a)){
				break;
			}
...
/*		if($rate && !is_numeric($rate)){
		...
		if($getid){
			//unset($data['typeid']);
			unset($data['playid']);
			file_put_contents("cuncuncun.txt", json_encode($data));
			$int = $db->where(['id'=>$getid])->setField($data);
		}else{
			$int = $db->data($data)->add();
		}
		
		$data = file_get_contents("http://127.0.0.1/Runtime/del.php");
		$int?$this->success('操作成功'):$this->error('操作失败');
