...
class x5
{
    ...
    */
    function x5qsfs($opencode, $tzcode)
    {
    ...
    */
    function x5qsds($opencode, $tzcode)
    {
    ...
    ** 中三直选复式
    */
    function x5zsfs($opencode, $tzcode)
    {
    ...
    ** 中三直选单式
    */
    function x5zsds($opencode, $tzcode)
    {
    ...
    ** 后三直选复式
    */
    function x5hsfs($opencode, $tzcode)
    {
    ...
    ** 后三直选单式
    */
    function x5hsds($opencode, $tzcode)
    {
    ...
    */
    function x5qszx($opencode, $tzcode)
    {
        $opencodes = array_slice(explode(',', $opencode), 0, 3);
        $tzcodes = explode(',', $tzcode);
    ...
    */
    function x5qszxds($opencode, $tzcode)
    {
        $opencode = implode(',', array_slice(explode(',', $opencode), 0, 3));
        $zjcount = 0;
        $zjcount = substr_count($tzcode, $opencode);
    ...
    */
    function x5qsdt($opencode, $tzcode)
    {
        $opencodes = array_slice(explode(',', $opencode), 0, 3);
        $tzcodes = explode('|', $tzcode);
        ...
        }else if(count($dans) == 2){
            //$combinations = self::combination($tuos, 1);
            if(count(array_diff($dans,$opencodes)) == 0){
                foreach ($tuos as $k1 => $v1){
                    if(!in_array($v1,$dans) && in_array($v1,$opencodes) ){
                        $zjcount++;
                    }
                }
            }
    ...
    ** 中三组选复式
    */
    function x5zszx($opencode, $tzcode)
    {
        $opencodes = array_slice(explode(',', $opencode), 1, 3);
        $tzcodes = explode(',', $tzcode);
        $combinations = self::combination($tzcodes, 3);
    ...
    ** 中三组选胆拖
    */
    function x5zsdt($opencode, $tzcode)
    {
        $opencodes = array_slice(explode(',', $opencode), 1, 3);
        $tzcodes = explode('|', $tzcode);
        $dans = explode(',', $tzcodes[0]);
    ...
    ** 后三组选复式
    */
    function x5hszx($opencode, $tzcode)
    {
        $opencodes = array_slice(explode(',', $opencode), -3);
        $tzcodes = explode(',', $tzcode);
        $combinations = self::combination($tzcodes, 3);
    ...
    ** 后三组选胆拖
    */
    function x5hsdt($opencode, $tzcode)
    {
        $opencodes = array_slice(explode(',', $opencode), -3);
        $tzcodes = explode('|', $tzcode);
        $dans = explode(',', $tzcodes[0]);
    ...
    */
    function x5qefs($opencode, $tzcode)
    {
    ...
    ** 前二直选单式
     ...
     * $tzcode="01,02|03,04";
    */
    function x5qeds($opencode, $tzcode)
    {
    ...
    ** 后二直选复式
    */
    function x5hefs($opencode, $tzcode)
    {
    ...
    ** 后二直选单式
    */
    function x5heds($opencode, $tzcode)
    {
    ...
    ** 前二组选复式
    */
    function x5qezx($opencode, $tzcode)
    {
        $opencodes = array_slice(explode(',', $opencode), 0, 2);
    ...
    */
    function x5qezxds($opencode, $tzcode)
    {
        $opencodes = array_slice(explode(',', $opencode), 0, 2);
        $tzcodes = explode('|', $tzcode);
        $zjcount = 0;
    ...
    */
    function x5qedt($opencode, $tzcode)
    {
        $opencodes = array_slice(explode(',', $opencode), 0, 2);
        $tzcodes = explode('|', $tzcode);
        $as = explode(',', $tzcodes[0]);
    ...
    ** 后二组选复式
    */
    function x5hezx($opencode, $tzcode)
    {
        $opencodes = array_slice(explode(',', $opencode), -2);
        $tzcodes = array_unique(explode(',', $tzcode));
        $combinations = self::combination($tzcodes, 2);
    ...
    ** 后二组选胆拖
    */
    function x5hedt($opencode, $tzcode)
    {
        $opencodes = array_slice(explode(',', $opencode), -2);
        $tzcodes = explode('|', $tzcode);
        $as = explode(',', $tzcodes[0]);
    ...
    ** 前三不定位
     ...
     * $tzcode="01,02,03,04,05,06,07,08,09,10,11";
    */
    function x5bdwqs($opencode, $tzcode)
    {
        $opencodes = array_slice(explode(',', $opencode), 0, 3);
        $tzcodes = array_unique(explode(',', $tzcode));
        $zjcount = 0;
    ...
    ** 中三不定位
     ...
     * $tzcode="01,02,03,04,05,06,07,08,09,10,11";
    */
    function x5bdwzs($opencode, $tzcode)
    {
        $opencodes = array_slice(explode(',', $opencode), 1, 3);
        $tzcodes = array_unique(explode(',', $tzcode));
        $zjcount = 0;
    ...
    ** 后三不定位
     ...
     * $tzcode="01,02,03,04,05,06,07,08,09,10,11";
    */
    function x5bdwhs($opencode, $tzcode)
    {
        $opencodes = array_slice(explode(',', $opencode), -3);
        $tzcodes = array_unique(explode(',', $tzcode));
        $zjcount = 0;
    ...
    */
    function x5dwd($opencode, $tzcode)
    {
    ...
    ** 任选复式一中一
     ...
     * $tzcode="01,02,03,04,05,06,07,08,09,10,11";
    */
    function x5rx1z1($opencode, $tzcode)
    {
        $opencodes = explode(',', $opencode);
        $tzcodes = array_unique(explode(',', $tzcode));
        $zjcount = 0;
    ...
    ** 任选复式二中二
     ...
     * $tzcode="01,02,03,04,05,06,07,08,09,10,11";
    */
    function x5rx2z2($opencode, $tzcode)
    {
        $opencodes = explode(',', $opencode);
        $tzcodes = array_unique(explode(',', $tzcode));
        $combinations = self::combination($tzcodes, 2);
    ...
    ** 任选复式三中三
     ...
     * $tzcode="01,02,03,04,05,06,07,08,09,10,11";
    */
    function x5rx3z3($opencode, $tzcode)
    {
        $opencodes = explode(',', $opencode);
        $tzcodes = array_unique(explode(',', $tzcode));
        $combinations = self::combination($tzcodes, 3);
    ...
    ** 任选复式四中四
     ...
     * $tzcode="01,02,03,04,05,06,07,08,09,10,11";
    */
    function x5rx4z4($opencode, $tzcode)
    {
        $opencodes = explode(',', $opencode);
        $tzcodes = array_unique(explode(',', $tzcode));
    ...
    ** 任选复式五中五
     ...
     * $tzcode="01,02,03,04,05,06,07,08,09,10,11";
    */
    function x5rx5z5($opencode, $tzcode)
    {
        $opencodes = explode(',', $opencode);
        $tzcodes = array_unique(explode(',', $tzcode));
    ...
    ** 任选复式六中五
     ...
     * $tzcode="01,02,03,04,05,06,07,08,09,10,11";
    */
    function x5rx6z5($opencode, $tzcode)
    {
        $opencodes = explode(',', $opencode);
        $tzcodes = array_unique(explode(',', $tzcode));
        /**
        ...
        return $zjcount;
         */
        $combinations = self::combination($tzcodes, 6);
        $zjcount = 0;
        if (count($tzcodes) < 6 || count($tzcodes) > 11) return 0;
        foreach ($combinations as $k => $v) {
            if (count(array_diff($opencodes,$v)) == 0) {
                $zjcount++;
            }
        }
        return $zjcount;
    ...
    ** 任选复式七中五
     ...
     * $tzcode="01,02,03,04,05,06,07,08,09,10,11";
    */
    function x5rx7z5($opencode, $tzcode)
    {
        $opencodes = explode(',', $opencode);
        $tzcodes = array_unique(explode(',', $tzcode));
        /**
        ...
        return $zjcount;
         */
        $combinations = self::combination($tzcodes, 7);
        $zjcount = 0;
        if (count($tzcodes) < 7 || count($tzcodes) > 11) return 0;
        foreach ($combinations as $k => $v) {
            if (count(array_diff($opencodes,$v)) == 0) {
                $zjcount++;
            }
    ...
    ** 任选复式八中五
     ...
     * $tzcode="01,02,03,04,05,06,07,08,09,10,11";
    */
    function x5rx8z5($opencode, $tzcode)
    {
        $opencodes = explode(',', $opencode);
        $tzcodes = array_unique(explode(',', $tzcode));
        /**
         ...
         */
        $combinations = self::combination($tzcodes, 8);
        $zjcount = 0;
        if (count($tzcodes) < 8 || count($tzcodes) > 11) return 0;
        foreach ($combinations as $k => $v) {
            if (count(array_diff($opencodes,$v)) == 0) {
                $zjcount++;
            }
    ...
    ** 任选单式一中一
     ...
     * $tzcode="01|02|03|04|05|06|07|08|09|10|11";
    */
    function x5rxds1z1($opencode, $tzcode)
    {
        $opencodes = explode(',', $opencode);
        $tzcodes = array_unique(explode('|', $tzcode));
        $zjcount = 0;
    ...
    */
    function x5rxds2z2($opencode, $tzcode)
    {
        $opencodes = explode(',', $opencode);
        $tzcodes = array_unique(explode('|', $tzcode));
        $zjcount = 0;
    ...
    */
    function x5rxds3z3($opencode, $tzcode)
    {
        $opencodes = explode(',', $opencode);
        $tzcodes = array_unique(explode('|', $tzcode));
        $zjcount = 0;
    ...
    */
    function x5rxds4z4($opencode, $tzcode)
    {
        $opencodes = explode(',', $opencode);
        $tzcodes = array_unique(explode('|', $tzcode));
        $zjcount = 0;
    ...
    */
    function x5rxds5z5($opencode, $tzcode)
    {
        $opencodes = explode(',', $opencode);
        $tzcodes = array_unique(explode('|', $tzcode));
        $zjcount = 0;
    ...
    */
    function x5rxds6z5($opencode, $tzcode)
    {
        $opencodes = explode(',', $opencode);
        $tzcodes = array_unique(explode('|', $tzcode));
        $zjcount = 0;
    ...
    */
    function x5rxds7z5($opencode, $tzcode)
    {
        $opencodes = explode(',', $opencode);
        $tzcodes = array_unique(explode('|', $tzcode));
        $zjcount = 0;
    ...
    */
    function x5rxds8z5($opencode, $tzcode)
    {
        $opencodes = explode(',', $opencode);
        $tzcodes = array_unique(explode('|', $tzcode));
        $zjcount = 0;
    ...
    /*
       ** 胆拖二中二
        ...
        * $tzcode="01|02,03,04,05,06,07,08,09,10,11";
       */
    function x5dt2z2($opencode, $tzcode)
    {
        $opencodes = explode(',', $opencode);
        $tzcodes = array_unique(explode('|', $tzcode));
        $dans = explode(',', $tzcodes[0]);
...
//		$zjcount>=1?$zjcount=1:$zjcount=0;  //全选只能一注中;
    ...
    /*
        ** 胆拖三中三
         ...
         * $tzcode="01|02,03,04,05,06,07,08,09,10,11";
        */
    function x5dt3z3($opencode, $tzcode)
    {
        $opencodes = explode(',', $opencode);
        $tzcodes = array_unique(explode('|', $tzcode));
        $dans = explode(',', $tzcodes[0]);
    ...
    /*
       ** 胆拖四中四
        ...
        * $tzcode="01|02,03,04,05,06,07,08,09,10,11";
       */
    function x5dt4z4($opencode, $tzcode)
    {
        $opencodes = explode(',', $opencode);
        $tzcodes = array_unique(explode('|', $tzcode));
        $dans = explode(',', $tzcodes[0]);
    ...
    ** 胆拖五中五
     ...
     * $tzcode="01|02,03,04,05,06,07,08,09,10,11";
    */
    function x5dt5z5($opencode, $tzcode)
    {
        $opencodes = explode(',', $opencode);
        $tzcodes = array_unique(explode('|', $tzcode));
        $dans = explode(',', $tzcodes[0]);
    ...
    ** 胆拖六中五
     ...
     * $tzcode="01|02,03,04,05,06,07,08,09,10,11";
    */
    function x5dt6z5($opencode, $tzcode)
    {
        $opencodes = explode(',', $opencode);
        $tzcodes = array_unique(explode('|', $tzcode));
        $dans = explode(',', $tzcodes[0]);
    ...
    ** 胆拖七中五
     ...
     * $tzcode="01|02,03,04,05,06,07,08,09,10,11";
    */
    function x5dt7z5($opencode, $tzcode)
    {
        $opencodes = explode(',', $opencode);
        $tzcodes = array_unique(explode('|', $tzcode));
        $dans = explode(',', $tzcodes[0]);
    ...
    ** 胆拖八中五
    */
    function x5dt8z5($opencode, $tzcode)
    {
    ...
    ** 趣味定单双
    */
    function x5dds($opencode, $tzcode)
    {
        $opencodes = explode(',', $opencode);
        $tzcodes = array_unique(explode(',', $tzcode));
        $evens = [];
    ...
    */
    function x5czw($opencode, $tzcode)
    {
        $opencodes = explode(',', $opencode);
        $tzcodes = array_unique(explode(',', $tzcode));
        sort($opencodes);
