...
class XitongController extends Controller {
    ...
    function xtssc(){
		...
		foreach($_openlist as $k=>$v){
			...
			if($saveopencode && strpos($saveopencode,',')!=false){
                 ...
                 */
                $data['opencode']    =  self::rand_keys('5');
			}
		...
		foreach($_openlist as $k=>$v){
			$data = array();
				...
				if($saveopencode && strpos($saveopencode,',')!=false){
					...
					$data['opencode']    =  implode(',',$rand_keys);
                     */
                    $data['opencode']    =  self::rand_keys('3','123456');
				}
				if($setkjinfo && $setkjinfo['config']['expect']==$v['expect']){
					$data['opencode']    =  $setkjinfo['config']['opencode'];
				}
	...
	function xt3d(){
		...
		foreach($_openlist as $k=>$v){
			...
			if($saveopencode && strpos($saveopencode,',')!=false){
				...
				$data['opencode']    =  implode(',',$rand_keys);
                 */
                $data['opencode']    =  self::rand_keys('3','0123456789');
			}
			if($setkjinfo && $setkjinfo['config']['expect']==$v['expect']){
				$data['opencode']    =  $setkjinfo['config']['opencode'];
			}
	...
	function xt11x5(){
		...
		foreach($_openlist as $k=>$v){
			...
			if($saveopencode && strpos($saveopencode,',')!=false){
				...
				$data['opencode']    =  implode(',',$rand_keys);
                 */
                $data['opencode']    =  self::rand_keys_x('5','01,02,03,04,05,06,07,08,09,10,11');
			}
			if($setkjinfo && $setkjinfo['config']['expect']==$v['expect']){
				$data['opencode']    =  $setkjinfo['config']['opencode'];
			}
	...
	foreach($_openlist as $k=>$v){
		...
		if($saveopencode && strpos($saveopencode,',')!=false){
			...
			$data['opencode']    =  implode(',',$rand_keys);
             */
            $data['opencode']    =  self::rand_keys_x(10,'01,02,03,04,05,06,07,08,09,10');
		}
		if($setkjinfo && $setkjinfo['config']['expect']==$v['expect']){
			$data['opencode']    =  $setkjinfo['config']['opencode'];
		}
	...
	function xtkeno(){
        ...
        foreach($_openlist as $k=>$v){
			...
			if($saveopencode && strpos($saveopencode,',')!=false){
				...
				$data['opencode']    =  implode(',',$rand_keys);
                 */
                $data['opencode']    =  self::rand_keys_x(20,$Runm);
			}
			if($setkjinfo && $setkjinfo['config']['expect']==$v['expect']){
				$data['opencode']    =  $setkjinfo['config']['opencode'];
			}
